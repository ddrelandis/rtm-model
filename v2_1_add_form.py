...
class BreastRadiometryModelReal:
    ...
    def create_anatomical_phantom(self, shape=(60, 80), tumor_radius=8, tumor_pos=None):
        """
        Создание анатомического фантома с соском на вершине и без нижней границы
        (грудь переходит в тело).
        """
        h, w = shape
        ...
        # =========================================================
        # 🎨 1. СОЗДАНИЕ ФОРМЫ ГРУДИ (БЕЗ НИЖНЕЙ ГРАНИЦЫ)
        # =========================================================
        
        top_y = int(h * 0.05)  # Самая верхняя точка груди (сосок)
        
        # Радиусы для формирования естественной кривой
        breast_width_top = w * 0.06   # Очень узко у верха (сосок)
        breast_width_mid = w * 0.35   # Средняя часть
        breast_width_base = w * 0.4  # Широко у основания (переход в тело)
        
        breast_mask = np.zeros(shape, dtype=bool)
        
        for yi in range(top_y, h):  # До самого низа изображения!
            # Нормализованная высота (0 вверху, 1 внизу)
            normalized_y = (yi - top_y) / (h - top_y)
            
            # Квадратичная интерполяция ширины для естественной формы
            if normalized_y < 0.25:
                # Верхняя часть - быстрое расширение от соска
                width_factor = breast_width_top + (breast_width_mid - breast_width_top) * (normalized_y / 0.25) ** 0.5
            elif normalized_y < 0.6:
                # Средняя часть - плавное расширение
                width_factor = breast_width_mid + (breast_width_base - breast_width_mid) * ((normalized_y - 0.25) / 0.35)
            else:
                # Нижняя часть - плавный переход в тело (ширина стабилизируется)
        ...
        # Сглаживание контуров (только боковых, не снизу)
        breast_mask = binary_dilation(breast_mask, iterations=1)
        breast_mask = gaussian_filter(breast_mask.astype(float), sigma=0.8) > 0.5
        
        # =========================================================
        # 🎯 2. СОСОК ПРЯМО НА ВЕРШИНЕ
        # =========================================================
        
        nipple_center_y = int(h * 0.15)  # Очень близко к верху
        nipple_center_x = int(w / 2.0)    # По центру
        ...
        # =========================================================
        # 🧊 3. ЗАПОЛНЕНИЕ ТКАНЯМИ
        # =========================================================
        
        # Область груди (без ареолы) - жировая ткань
        breast_body_mask = breast_mask & ~areola_mask
        
        # Нижняя часть (переход в тело) - отдельная зона
        body_transition_y = int(h * 0.70)
        body_mask = (y >= body_transition_y) & breast_mask
        
        # Основная часть груди
        main_breast_mask = breast_body_mask & (y < body_transition_y)
        
        # Заполнение основной части груди жировой тканью
        ...
        tumor_mask = tumor_mask & main_breast_mask
        ...
        if np.any(tumor_mask):
            ...
            print("⚠️ Опухоль не попала в область груди")
...
def plot_breast_anatomy(eps_map, breast_mask, areola_mask, nipple_mask, body_mask):
    """Визуализация анатомической структуры молочной железы"""
    fig, axes = plt.subplots(1, 2, figsize=(14, 5))
    
    eps_display = eps_map.copy()
    eps_display[~breast_mask] = np.nan
    im1 = axes[0].imshow(eps_display, cmap='viridis', vmin=0, vmax=60)
...
if __name__ == "__main__":
    ...
    print("="*70)
    print("🔬 МОДЕЛЬ РАДИОМЕТРИИ МОЛОЧНОЙ ЖЕЛЕЗЫ (БЕЗ НИЖНЕЙ ГРАНИЦЫ)")
    print("="*70)
    ...
    print("\n✅ Все графики сохранены в файлы 01_*.png ... 07_*.png")
    print("="*70)