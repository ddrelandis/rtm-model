...
class BreastRadiometryModel3D:
    ...
    def compute_sensitivity_kernel_3d(self, mask, ant_pos_3d, direction=None):
        """
        Вычисляет 3D ядро чувствительности для одной антенны.
        
        direction: единичный вектор [dz, dy, dx] в координатах массива.
                Если None — используется значение по умолчанию [1, 0, 0] (вниз по Z).
        
        Физика: антенна чувствительна только к точкам "перед" ней
        (проекция вектора от антенны к точке на direction > 0).
        """
        d, h, w = mask.shape
        z, y, x = np.ogrid[:d, :h, :w]
        
        # Вектор от антенны к каждой точке объёма
        vec_z = z - ant_pos_3d[0]
        vec_y = y - ant_pos_3d[1]
        vec_x = x - ant_pos_3d[2]
        
        # ⭐ НАПРАВЛЕННОЕ ЯДРО
        if direction is not None:
            # Нормируем direction (на случай если он не единичный)
            dir_norm = np.linalg.norm(direction)
            if dir_norm > 1e-6:
                dz, dy, dx = direction / dir_norm
            else:
                dz, dy, dx = 1.0, 0.0, 0.0
            
            # Проекция вектора "антенна→точка" на направление антенны
            # Положительная = точка впереди антенны (антенна её "видит")
            projection = vec_z * dz + vec_y * dy + vec_x * dx
            
            # Поперечное расстояние (перпендикуляр к направлению)
            # perp² = |v|² - proj²
            dist_sq = vec_z**2 + vec_y**2 + vec_x**2
            perp_sq = np.maximum(dist_sq - projection**2, 0)
            
            # Параметры гауссианы
            sigma_perp = 20.0   # Поперечное разрешение
            sigma_along = 45.0  # Продольное разрешение (глубина проникновения)
            
            # ⭐ Направленная гауссиана: работает ТОЛЬКО для projection > 0
            # Используем max(0, projection), чтобы точки позади антенны имели вес 0
            along_valid = np.maximum(projection, 0)
            
            kernel = np.exp(-(perp_sq) / (2 * sigma_perp**2) 
                        - (along_valid**2) / (2 * sigma_along**2))
            
            # ⭐ Жёсткое обнуление: антенна НЕ ВИДИТ точки позади себя
            kernel[projection < 0] = 0.0
            
        else:
            # Fallback: классический ненаправленный случай
            dist_xy = np.sqrt(vec_y**2 + vec_x**2)
            dist_z = np.abs(vec_z)
            sigma_xy = 12.0
            sigma_z = 25.0
    ...
    def forward_scan_3d(self, temp_map, eps_map, mask, scan_positions_3d, 
                        antenna_directions=None):
        """
        Прямое сканирование с учётом направлений антенн.
        antenna_directions: список векторов [dz, dy, dx] или None
        """
        ...
        # ⭐ Добавляем индикаторы направлений в вывод
        if antenna_directions:
            unique_dirs = len(set(tuple(d) for d in antenna_directions))
            print(f"   🧭 Используется {unique_dirs} различных направлений")
        
        for i, pos in enumerate(scan_positions_3d):
            # Получаем направление для текущей антенны (или None)
            direction = antenna_directions[i] if antenna_directions else None
            
            kernel = self.compute_sensitivity_kernel_3d(mask, pos, direction=direction)
    ...
    def reconstruct_3d(self, Tb_data, emissivity_avg, scan_positions_3d, 
                    shape, mask, antenna_directions=None):
        """
        3D Реконструкция с учётом направлений антенн.
        """
