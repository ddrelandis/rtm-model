...
class BreastRadiometryModelReal:
    ...
    def create_anatomical_phantom(self, shape=(60, 80), tumor_radius=8, tumor_pos=None):
        ...
        for _ in range(n_lobules):
            ...
            lobule_size = np.random.randint(4, 10)
            yy, xx = np.ogrid[:h, :w]
        ...
        for _ in range(n_fibers):
            ...
            fiber_length = np.random.randint(10, 25)
            ...
            for l in range(fiber_length):
                ...
                if 0 <= px < w and 0 <= py < h:
                    fiber_region = (x - px)**2 + (y - py)**2 <= 2**2
...
if __name__ == "__main__":
    ...
    print("="*70)
    print("🔬 МОДЕЛЬ РАДИОМЕТРИИ МОЛОЧНОЙ ЖЕЛЕЗЫ (ВИДИМАЯ СТРУКТУРА)")
    print("="*70)
    
    birads = 'B'
    model = BreastRadiometryModelReal(freq_ghz=3.0, resolution_mm=4, birads_category=birads)
