...
class BreastRadiometryModelReal:
    def __init__(self, freq_ghz=3.5, resolution_mm=5):
        ...
        self.tumor_center = None  # Инициализация атрибута
    ...
    def create_anatomical_phantom(self, shape=(60, 80), tumor_radius=8, tumor_pos=None):
        """
        Создание анатомического фантома.
        tumor_pos: кортеж (y, x) для ручной установки координат опухоли.
        """
        h, w = shape
        ...
        # 3. Фиброгландулярная ткань (кластерами)
        n_clusters = 60
        ...
        # 4. ОПУХОЛЬ: заданная или случайная позиция
        ...
        if tumor_pos is not None:
            ...
            if 0 <= ty < h and 0 <= tx < w:
                if ellipse_mask[ty, tx]:
                    print(f"✅ Опухоль создана в заданной позиции: Y={ty}, X={tx}")
                else:
                    print(f"⚠️ Позиция ({ty}, {tx}) вне груди! Коррекция...")
                    y_coords, x_coords = np.where(ellipse_mask)
                ...
                tumor_pos = None # Переключаемся на случайную генерацию
        
        if tumor_pos is None:
            # Случайная позиция внутри груди
...
if __name__ == "__main__":
    # Настройка стиля графиков (с защитой от ошибок)
    try:
    ...
    h, w = temp_true.shape
    scan_y = int(h * 0.40)
    x_pos = np.linspace(int(w * 0.2), int(w * 0.8), 25, dtype=int)
