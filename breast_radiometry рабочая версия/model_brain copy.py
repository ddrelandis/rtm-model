...
class BrainRadiometryModel3D:
    ...
    def create_realistic_head_shape(self, shape, scale=1.0):
        """
        Создает реалистичную форму головы без жестких обрезаний.
        """
        d, h, w = shape
        z, y, x = np.ogrid[:d, :h, :w]
        center_z, center_y, center_x = d * 0.5, h * 0.5, w * 0.5
        
        # Базовый эллипсоид — используем почти весь объём
        head_rz = d * 0.46 * scale
        head_ry = h * 0.46 * scale
        head_rx = w * 0.46 * scale
        ...
        # Плавная модификация формы: голова чуть более вытянута спереди-сзади
        # и слегка сплюснута по бокам
        shape_mod = 1.0 - 0.1 * np.exp(-(z_norm**2 + y_norm**2) / 0.5)
        
        # Формируем маску головы (полный эллипсоид с модификацией)
        head_mask = ((z_norm**2 + y_norm**2 + x_norm**2) * shape_mod) <= 1.0
    ...
    def create_cortical_folding(self, brain_mask, shape):
        ...
        """
        Создает реалистичные извилины и борозды коры с использованием шума Перлина.
        """
        d, h, w = shape
        z, y, x = np.ogrid[:d, :h, :w]
        
        # Создаём шум Перлина для моделирования извилин
        from noise import pnoise3
        noise_field = np.zeros(shape)
        scale = 100.0  # Масштаб шума (меньше значение - больше деталей)
        octaves = 6
        persistence = 0.5
        lacunarity = 2.0
        ...
        # Создаем борозды (низкие значения шума)
        sulci_mask = noise_field < 0.3
        sulci_mask &= brain_mask
        
        # Создаем извилины (высокие значения шума)
        gyri_mask = noise_field > 0.7
        gyri_mask &= brain_mask
        
        # ✅ ИСПРАВЛЕНИЕ: Правильное использование ogrid с broadcasting
        central_sulcus = (np.abs(x - w//2) < max(3, int(5*scale))) & (y > int(h*0.30)) & (y < int(h*0.50))
        sylvian_fissure = (y > int(h*0.40)) & (y < int(h*0.55)) & (x > int(w*0.35)) & (x < int(w*0.65))
        parieto_occipital = (z > int(d*0.60)) & (np.abs(x - w//2) < max(3, int(5*scale)))
        
        # Добавляем основные борозды
        sulci_mask |= central_sulcus
        sulci_mask |= sylvian_fissure
        sulci_mask |= parieto_occipital
        
        # Создаем контур коры с извилинами
        cortex_mask = brain_mask & ~sulci_mask
        
        return cortex_mask, sulci_mask, noise_field
    ...
    def create_cerebellum(self, shape, brain_mask):
        """
        Создает реалистичную форму мозжечка с дольками.
        """
        d, h, w = shape
        z, y, x = np.ogrid[:d, :h, :w]
        
        # Центр мозжечка
        cerebellum_center_z = int(d * 0.85)
        cerebellum_center_y = int(h * 0.5)
        cerebellum_center_x = int(w * 0.5)
        
        # Размеры мозжечка
        cerebellum_rz = int(d * 0.10)
        cerebellum_ry = int(h * 0.15)
        cerebellum_rx = int(w * 0.18)
        
        # Базовая форма мозжечка
        ...
        # Добавляем дольки мозжечка (folia)
        if self.use_real_anatomy:
            # Для реалистичной модели используем перлин-шум
            from noise import pnoise3
            folia_noise = np.zeros(shape)
            scale = 50.0
            
            for i in range(d):
                for j in range(h):
                    for k in range(w):
                        folia_noise[i, j, k] = pnoise3(
                            i / scale, 
                            j / scale, 
                            k / scale, 
                            octaves=6
            ...
            # Нормализуем
            folia_noise = (folia_noise - np.min(folia_noise)) / (np.max(folia_noise) - np.min(folia_noise))
            
            # Дольки - это области с низким шумом
            folia_mask = folia_noise < 0.3
            cerebellum_mask &= folia_mask
        else:
            # Более простой метод для ускорения
            folia = np.sin((z - cerebellum_center_z) * 0.8) * 0.5 + 0.5
            cerebellum_mask &= (np.random.rand(d, h, w) < 0.7 + 0.3 * folia)
        
        # Привязываем к мозгу
        cerebellum_mask &= brain_mask
    ...
    def create_anatomical_brain_phantom(self, shape=(100, 140, 140), tumor_radius=8, tumor_pos=None):
        ...
        # 5. Мозг с извилинами
        cortex_mask, sulci_mask, noise_field = self.create_cortical_folding(brain_surface, shape)
        white_matter = brain_surface & ~cortex_mask
