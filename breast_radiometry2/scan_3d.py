...
ADD_NOISE = True                # шум в измерениях (как в реальном приборе)
NOISE_STD_K = 0.1             # отклонение шума (в Кельвинах)
...
def validate_antenna_positions(scan_positions, shape, mask, search_radius=10):
    ...
    for pos in scan_positions:
        ...
        if not (0 <= z < d and 0 <= y < h and 0 <= x < w):
            print(f"Позиция ({z}, {y}, {x}) вне сетки. Пропущено.")
            continue
...
def create_3d_visualization(temp_recon, temp_true, breast_mask, tumor_mask,
    ...
    # контур истинной опухоли (черный)
    ...
    # изосурфейс горячей зоны в реконструкции (> 37.5°C)
    try:
        ...
        print(f"Не удалось построить горячую зону: {e}")
    ...
    # антенны (цветные точки по Z-уровням)
