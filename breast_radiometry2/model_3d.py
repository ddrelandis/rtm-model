...
class BreastRadiometryModel3D:
        ...
        if tumor_pos is not None:
            ...
            if not (0 <= tumor_z < d and 0 <= tumor_y < h and 0 <= tumor_x < w):
                ...
                if len(gz) > 0:
                    ...
                    print("Не удалось скорректировать позицию. Генерация случайной позиции")
                    tumor_pos = None
    ...
    def compute_emissivity_3d(self, eps_map, mask=None):
        #расчет коэффициента излучения emissivity
    ...
    def compute_sensitivity_kernel_3d(self, mask, ant_pos_3d):
        #ядро чувствительности для одной антенны
        d, h, w = mask.shape
        z, y, x = np.ogrid[:d, :h, :w]
        ...
        #насколько "широко" и "глубоко" антенна "видит" температуру внутри ткани
        #ядро локализовано вокруг антенны
        sigma_xy = 8.0   #латеральное (поперечное) разрешение  
        sigma_z = 18.0   
    ...
    def forward_scan_3d(self, temp_map, eps_map, mask, scan_positions_3d):
        #расчет яркостной температуры (Tb).
